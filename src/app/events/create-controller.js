...
angular.module('inspinia')
    .controller('eventsCreateCtrl', ['$scope', '$state', 'NgMap', 'toaster', '$timeout', '$stateParams', '$rootScope', '$uibModal', '$log', 'FileUploader', 'configurationService', '$controller','eventsService','placesService', function ($scope, $state, NgMap, toaster, $timeout, $stateParams, $rootScope, $uibModal, $log, FileUploader, configurationService, $controller,eventsService,placesService) {
        $scope.events = {};
        $scope.notfoundcity = "";
        $scope.notfoundplace = "";
        $scope.appartment = {};
        ...
        $scope.save = function (model) {
            console.log(model);
            $scope.events=model;
            // var openingSplitTime=data.postdate.split('T') ;
            ...
            // $scope.bidOpeningDate =moment(data.postdate).format("YYYY-MM-DD");
            ...
            if (error == 0) {
                ...
                if ($state.current.breadcrumb.text == "Create") {
                    $scope.events.eventDate = $scope.bidOpeningDate+'T'+moment($scope.openingTime).format('HH:mm')+':00.000Z';
